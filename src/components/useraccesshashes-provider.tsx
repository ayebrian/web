...
const UserAccessHashesContext = createContext<UserAccessHashesContextValue | null>(null);

// We forcing this hook to skip initialization on server,
// to prevent hydration errors
export function UserAccessHashesProvider({children}: {children: React.ReactNode}) {
...
export function useUserAccessHashes() {
    ...
    if (!ctx) {
        throw new Error('useUserAccessHashes must be used inside UserAccessHashesProvider');
    }
    return ctx;
}
